...
from utils.tableau_object_helpers import get_users_by_name, get_groups_by_name, add_user_to_group
  ...
  with open('user_groups.csv') as file:
    ...
    for line in csvreader:
      ...
      if len(tableau_groups) ==1 and len(tableau_users)==1:
        add_user_to_group(AUTHENTICATION, SERVER, tableau_users[0],tableau_groups[0])
      else:
        print(f'Unable to update: {len(tableau_users)} users and {len(tableau_groups)}')
