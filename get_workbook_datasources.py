...
def get_workbook_datasources():
    ...
    with SERVER.auth.sign_in_with_personal_access_token(AUTHENTICATION):
        ...
        for workbook in all_workbooks:
            ...
            for connection in workbook.connections:
                log('[DS] workbook.name', workbook.name)
                ...
                connections_dict[workbook.id] = connection_dict
        ...
        for ds_id, ds_path in workbook_path_dict.items():
            with open(ds_path) as f:
                tree = ET.parse(f)
            root = tree.getroot()

            # Get human-readable name of workbook
            ds_name = connections_dict.get(ds_id).get('workbook_name')
    ...
    df.to_csv('./tableau_workbook_metadata.csv', encoding='utf-8', index=False)
    print(df)

    #
    # OUTPUT: JSON
    #
    # with open('./tableau_workbook_metadata.json', 'w') as f:
...
def download_workbooks(tableau_server, workbooks):
    ...
    for workbook in workbooks:
        ...
        if workbook.workbook_type not in text_filters:

            # Download Packaged Tableau Datasource file (.tdsx)
            zipped_ds_path = tableau_server.workbooks.download(
                workbook.id,
